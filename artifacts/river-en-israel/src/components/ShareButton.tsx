    ...
    <div ref={ref} className={cn("relative", className)}>
      <button
        ...
        className={cn(
          ...
          open
            ? "text-river-red"
            : "text-gray-400 hover:text-river-red"
        )}
        title="Compartir"
      >
      ...
      {open && (
        <div
          className="absolute z-50 bottom-full mb-2 right-0 bg-white rounded-xl shadow-2xl border border-gray-100 py-1.5 min-w-[170px]"
          onClick={(e) => e.stopPropagation()}
        >
          {opciones.map((op) => (
            <button
              key={op.label}
              onClick={(e) => { e.stopPropagation(); op.action(); if (op.label !== "Copiar enlace") setOpen(false); }}
              className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors text-left"
            >
              <span className={cn("flex-shrink-0", op.color)}>{op.icon}</span>
              <span className="text-gray-700 text-sm font-medium">{op.label}</span>
            </button>
          ))}
        </div>
      )}
